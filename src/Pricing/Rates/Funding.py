...
class Leg:
    ...
    def __init__(self,contract,type_rate:str):
    ...
    def precomputation(self,calc_date:ql.Date,model,data_rates:dict):
        rates,schedule,grid=data_rates['rates'],data_rates['schedule'][1:],data_rates['grid']
        ...
        self.fixgrid=np.array([max(0,cal.yearFraction(calc_date,x)) for x in self.fix_dates ])
        self.paygrid=np.array([cal.yearFraction(calc_date,x) for x in self.pay_dates])
        self.delta=np.array([self.paygrid[0]]+[x-y for x,y in zip(self.paygrid[1:],self.paygrid)] )

        T=cal.yearFraction(calc_date,self.maturity_date)
        if self.type_rate=='Overnight':
            ...
            idxs=[Functions.find_idx(schedule,d) for d in self.fix_dates]
            self.fwds=np.array([model.compute_deposit_from_rates(rates[:,i],t,'3M')
                                        for i,t in zip(idxs,self.fixgrid) ])
        ...
        if not hasattr(contract,'call_dates'):
            ...
            self.measure_change_factor=np.array([Base.compute_measure_change_factor(model,rates[:,i],t,T) 
                                        for i,t in zip(idxs,self.paygrid) ])
            
            self.df_continuation=[None]*len(contract.call_dates)
            for i,d in enumerate(contract.call_dates):
                fix_idx=Functions.find_idx(schedule,d)
                pay_idx=bisect(self.pay_dates,d)
                t=grid[fix_idx]
                self.df_continuation[i]=model.compute_discount_factor_from_rates(rates[:,idx],t,self.paygrid[pay_idx:])*model.DF(t)

    def compute_cashflows(self,spread:float):
        return (self.fwds+spread).T*np.tile(self.delta,(self.fwds.shape[1],1))
    

    def compute_values(self,spread:float):
        self.proba=np.ones(len(self.paygrid))
    ...
    def compute_values_for_early_redemption(self,stop_idxs:list[int],spread:float):
        ...
        self.coupons=np.zeros(len(self.paygrid))
        self.proba=np.ones(len(self.paygrid))

        # print(contract.call_idxs)
        # print(Counter(stop_idxs))

        for i,d in enumerate(self.pay_dates):
            cashflows=self.fwds[i]+spread
            if d <=contract.pay_dates[contract.call_idxs[0]]:
