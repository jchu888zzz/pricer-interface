...
def Calibration(curve,entity:Instruments.Credit_Single,cal=ql.Actual365Fixed(),display_details=False):
    ...
    spreads=np.interp(entity.standard_grid,interp_grid,entity.quotes,left=entity.quotes[0],right=entity.quotes[-1])
    ...
    Q=np.ones_like(entity.standard_grid)

    values=np.zeros_like(entity.standard_grid)
    tgrid=entity.standard_grid
    
    cl,fl,acc=0,0,0

    for i,(t1,t2) in enumerate(zip(tgrid,tgrid[1:])):
        h=t2-t1
        ZC=curve.Discount_Factor(t1)
        func=lambda x:( (1-entity.R)*(cl + func_cl(ZC,Q[i-1],x,h)) 
                        -spreads[i]*(fl + func_fl(ZC,Q[i-1],x,h) +acc + func_acc(ZC,Q[i-1],x,h)) )

        values[i]=optimize.brentq(func,-0.5,0.5)

        cl+=func_cl(ZC,Q[i-1],values[i],h)
        fl+=func_fl(ZC,Q[i-1],values[i],h)
        acc+=func_acc(ZC,Q[i-1],values[i],h)

        Q[i]=Q[i-1]*np.exp(-values[i]*h)
    
    if display_details:
        print(pd.DataFrame({'Schedule':entity.standard_schedule[:-1],'Default Proba':1-Q[:-1]}))
...
class Intensity:
    ...
    def compute_survival_proba(self,t:float):
        standard_tgrid=self.entity.standard_grid
        if t<=0:
            return 1
