...
def get_model(mkt_data, input_param):
    underlying = input_param.get('underlying1')
    return HullWhite.get_model(
        mkt_data['calc_date'], mkt_data, input_param['currency'], underlying
    )


def price_callable(contract, prep_model, structure_choice):
    risky_curve = prep_model['risky_curve']

    CallableFeature.precomputation(
        prep_model['calc_date'], contract, prep_model['model'],
        risky_curve, structure_choice=structure_choice)

    if structure_choice == "Bond":
        price = CallableFeature.compute_bond_price(contract)
        ...
        price = CallableFeature.compute_swap_price(contract)
...
def run_pricing(structure_choice="Swap"):
    ...
    price = price_callable(contract, prep_model, structure_choice)
